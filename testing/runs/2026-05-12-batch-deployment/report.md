...
- [x] Phase 2: `pvc deploy github_repos` completes without error
  - Existing `pvc-composer` environment detected and reused (no 20-min wait)
...
- [x] Phase 2: Cloud Run job for the pipeline exists after deploy
  - `pvc-job-github-repos` in `us-central1` confirmed (Ready)
...
- [x] Phase 3: DAG run completes successfully
  - `gcloud run jobs execute pvc-job-github-repos --wait` → exit 0
...
- [x] Phase 4: Second `pvc deploy` produces exactly one DAG (idempotent)
  - Single `github_repos.py` in Composer bucket; single `pvc-job-github-repos` Cloud Run job
...
- [x] Phase 4: GCS data files are untouched after `pvc undeploy`
  - Parquet file still at `gs://pvc-warehouse-quipu-data-generator/github_repos/github_repos/data/`
...
- `test_config.yml` has `catalog: local` which is correct for most scenarios but requires manual project.yml update for the batch-deployment scenario. The scenario notes document this, but it's an extra step that a real user following `new-pipeline` Step 10 would also hit (they'd need to run `pvc gcp setup` first). This is expected and documented — not a new finding.
