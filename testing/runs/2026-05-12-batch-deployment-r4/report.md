...
- [x] Phase 2: `pvc deploy` without `catalog: gcp` in `project.yml` exits with a clear error
...
- Terraform state persisted at `~/.pvc/terraform/pipelines/github_repos/`: ✓
- Composer environment reuse (`pvc-composer` already existed): ✓
...
- Second `pvc deploy` idempotent — Terraform import handled existing Cloud Run job: ✓
- `pvc undeploy --yes` ran `terraform destroy` and removed state dir: ✓
- Warehouse Parquet file survived `pvc undeploy` unchanged: ✓
...
- The fix to F-035 required manually re-uploading the corrected DAG file via `gsutil cp` during the test, since re-running `pvc deploy` would rebuild the container image (~3 min) unnecessarily.
  The Terraform-based approach updates the DAG file content in state, so a `terraform apply` alone (without rebuilding the image) would have been sufficient. Consider a `pvc redeploy-dag` subcommand that only re-applies Terraform without rebuilding the image.
...
1. F-035: Fixed in `pvc/gcp/batch_deploy.py` — `_dag_content()` now uses `CloudRunExecuteJobOperator`. Fix committed in this session.
